...
const NameConfirmationStep = ({ orgData, onComplete, onBack, canGoBack, stepData }) => {
  const [organizationName, setOrganizationName] = useState(stepData.organizationName || '');
  ...
  const validateNames = () => {
    ...
    if (organizationName.trim() !== confirmationName.trim()) {
      setError('Organization names do not match. Please ensure both entries are identical.');
      return false;
    }
    return true;
  };
  ...
  const isValid = organizationName.trim() === requiredOrgName && 
                  organizationName.trim() === confirmationName.trim();

  return (
    <div className="step-container">
      <h2 className="step-title">
        <Building size={24} style={{ color: 'var(--settings-accent-primary)', marginRight: '0.5rem' }} />
      ...
      <div className="step-content">
        <div style={{
          ...
          <div style={{ 
            ...
            <AlertTriangle size={20} style={{ color: 'var(--settings-warning-text)' }} />
            <h4 style={{ 
          ...
          <p style={{ 
            ...
            To prevent accidental deletion, you must type the exact organization name <strong>twice</strong>. 
            This ensures you are intentionally deleting the correct organization.
          </p>
        </div>
        ...
        <div style={{
          ...
          <strong>Note:</strong> The organization name is case-sensitive and must match exactly, 
          including any spaces, special characters, or punctuation.
        </div>
      </div>
