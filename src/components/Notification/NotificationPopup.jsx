...
const NotificationPopup = ({ open, onClose, notification, onMarkRead, onDelete }) => {
  const { updateCSSVariables } = useThemeStyles();
  React.useEffect(() => { updateCSSVariables(); }, [updateCSSVariables]);
  ...
  return (
    <Dialog open={open} onClose={onClose} maxWidth="sm" fullWidth>
      <Box className="maintenance-popup-container" style={{ background: `${config.color}05` }}>
        <DialogTitle>
          <Box display="flex" alignItems="center">
            <Avatar src={notification.profile_picture_url} sx={{ mr: 2, bgcolor: `${config.color}20`, color: config.color }} />
        ...
        <DialogContent>
          {/* Show type label */}
          <Typography variant="caption" sx={{ color: config.color, fontWeight: 500, mb: 1 }}>
            {config.label}
          </Typography>
        ...
        <DialogActions>
          {!notification.is_read && (
            <Button onClick={() => onMarkRead(notification.notification_id)} color="primary">
              Mark as Read
            </Button>
          )}
          <Button onClick={() => onDelete(notification.notification_id)} color="error">
            Delete
          </Button>
